...
interface ITopMemberLateStatisticExtend extends ITopMemberLateStatistic {
  key: React.Key;
  index: number;
}
...
const MemberStatistic = () => {
  ...
  const [form] = useForm();
  const filteredData: ITopMemberLateStatisticExtend[] = topCategoryList.map(
    (t, index) => {
      return {
        ...t,
        index: index + 1,
        ...
        <Form form={form} onFinish={onFinish}>
          <div className="flex gap-2">
            ...
            <Form.Item name={"search"}>
              <Input size="large" placeholder="Search for member..." />
            </Form.Item>
            <Button
              size="large"
              htmlType="submit"
