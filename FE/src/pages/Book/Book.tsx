...
import { Alert, Button, Input, Modal, Spin, Table, Tooltip } from "antd";
...
const Book = () => {
  ...
  const claims = useSelector((state: RootState) => state.auth.user?.claims);
  ...
  const columns: TableColumnsType<IBookExtend> = [
    ...
    },
    {
      ...
      render: (isbn: string) => {
        return (
          <Tooltip title={isbn}>
            <div className="text-wrap line-clamp-1">{isbn}</div>
          </Tooltip>
        );
      },
    },
    {
      ...
      render: (title: string) => {
        return (
          <Tooltip title={title}>
            <div className="max-w-[300px] text-wrap line-clamp-2">{title}</div>
          </Tooltip>
        );
      },
    },
